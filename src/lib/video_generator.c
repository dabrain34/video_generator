...
int video_generator_update(video_generator* g) {
  ...
  for (i = start_y; i < (uint32_t)(start_y + nlines); ++i) {
    if(g->pixel_size_in_bytes == 2) {
      for(k = 0; k < g->width; k++) {
        g->y[k + i * g->width] = (uint8_t)((yc >> 8) & 0xFF);
        g->y[k + i * g->width + 1] = (uint8_t)(yc & 0xFF);
      }
    } else {
  ...
  for (i = start_y; i < end_y; ++i) {
    if(g->pixel_size_in_bytes == 2) {
      for(k = 0; k < stride; k++) {
        g->u[k + i * stride] = (uint8_t)((uc >> 8) & 0xFF);
        g->u[k + i * stride + 1] = (uint8_t)(uc & 0xFF);
        g->v[k + i * stride] = (uint8_t)((vc >> 8) & 0xFF);
        g->v[k + i * stride + 1] = (uint8_t)(vc & 0xFF);
      }
    } else {
  ...
  for (j = y; j < (uint32_t)(y + h); ++j) {
    if(gen->pixel_size_in_bytes == 2) {
      for(k = 0; k < w * gen->pixel_size_in_bytes; k+=1) {
        gen->y[k + j * gen->width + x] =(uint8_t)((yc >> 8) & 0xFF);
        gen->y[k + j * gen->width + x + 1] = (uint8_t)(yc & 0xFF);
      }
    } else {
  ...
  for (j = yy; j < (yy + hh); ++j) {
    if (gen->pixel_size_in_bytes == 2) {
      for(k = 0; k < ww * gen->pixel_size_in_bytes; k+=1) {
        gen->u[k + j * half_w + xx] = (uint8_t)((uc >> 8) & 0xFF);
        gen->u[k + j * half_w + xx + 1] = (uint8_t)(uc & 0xFF);
        gen->v[k + j * half_w + xx ] = (uint8_t)((vc >> 8) & 0xFF);
        gen->v[k + j * half_w + xx + 1] = (uint8_t)(vc & 0xFF);
      }
    } else {
      memset(gen->u + j * half_w + xx,  (uint8_t)uc, (ww));
      memset(gen->v + j * half_w + xx,  (uint8_t)vc, (ww));
    }
  }
